(function () {
    ...
    const currentDayNumber = dayNumber > 0 && dayNumber < 8 ? dayNumber : 1;
    ...
    let periodAmount = 4 // кількість періодів в акції, треба для кешування інфи з табли
    ...
    function initSpin(){
        ...
        sendSpinRequest().then(res => {
            ...
            if (prize === 'points50') openPopupByAttr('prize50Points', true);
            ...
            request(`/favuser/${userId}`).then(user => {
                setUserProgress(user, activeDayInWeekNumber, user.spinAllowed)
    ...
    function setUserProgress(userData, dayNum, isSpinDone) {
        ...
        if(!isCurrentDay){
            ...
            challangeBtnPointer.classList.add("hide");
            challangeDepositBtn.classList.remove("_lock");
        }
        if(isCurrentDay){
    ...
    function getPrizeNameKey(prize) {

        const prizeMap = {
            wheelBubbleSpans: 'prize50Points',
            iphone: 'prizeIphone',
            macBook: 'prizeLaptop',
            airPods: 'prizeAirpods',
            appleWatch: 'prizeWatch',
            rozetkaGiftCard: 'prizeGift',
            fs50: 'prize50FsGateOlymp',
            points50: 'prize50Points',
            fs25_3oaks: 'prize25FsLuckyPenny',
            fs25_playson: 'prize25FsCoinStrike',
            fs40: 'prize40FsPirots3',
            fs25_bgaming: 'prize25FsSnoopDogg',
            fs25_pushGaming: 'prize25FsBigBamboo',
            fs10: 'prize10FsHitSlot',
            fs30: 'prize30FsShinigCrown',
            fs25_hacksaw: 'prize25FsLeBandit',
            fs20: 'prize20FsLuckOfPanda',
            nothing: 'unluckyPopupTitle',
        };

        return prizeMap[prize] || null;
    }
    ...
    function autoAddDataButtons() {
        ...
        gameLists.forEach(list => {
            ...
            const items = list.querySelectorAll('.games__list-item');

            items.forEach((item, index) => {
                const gameNumber = index + 1;
                const buttonName = `week${week}Game${gameNumber}`;

                const linkEl = item.querySelector('.games__list-link');
                if (linkEl) {
                    linkEl.setAttribute('data-button', buttonName);
                    linkEl.setAttribute('data-promo', 'sam_chill_ua_14072025');
                }

                const btnEl = item.querySelector('.games__list-item-btn');
                if (btnEl) {
                    btnEl.setAttribute('data-button', buttonName);
                    btnEl.setAttribute('data-promo', 'sam_chill_ua_14072025');
                }
            });
        });
    }
    ...
    function displayBetsHistory(bets) {
        ...
        bets = bets.sort((a, b) => new Date(b.date) - new Date(a.date));
...
<!--            <div class="content-status" data-translate="myBetStatus"></div>-->
        ...
        bets.forEach(spin => {
            ...
            let prizeName = getPrizeNameKey(spin.name)
            ...
            if (status) {
                ...
                spinElement.innerHTML = `
                    ...
                    <span class="content-prize">${translateKey(prizeName)}</span>
<!--                    <span class="content-status ${statusClass}"></span>-->
        ...
        function quickCheckAndRender() {
            ...
            tableTabs.forEach((tab, index) => {
                ...
                tab.addEventListener('click', (e) => {
                    ...
                    if(weekNum > activeWeek || tab.classList.contains('active')) return
                    tableTabs.forEach(t => t.classList.remove('active'));
                    tab.classList.add('active');
                    renderUsers(weekNum, tableData)
                })
    ...
    function renderUsers(weekNum, userData) {
        ...
        populateUsersTable(userData.users, userId, weekNum);
    }
    ...
    function loadUsers() {
        ...
        for (let i = 1; i <= periodAmount; i++) {
            ...
            const req = request(`/users/${week}`).then(data => {
    ...
    loadTranslations().then(loadUsers).then(init)
